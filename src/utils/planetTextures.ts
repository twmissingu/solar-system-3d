...
function generateEarthTexture(): HTMLCanvasElement {
  ...
  for (let y = 0; y < 512; y++) {
    for (let x = 0; x < 1024; x++) {
      ...
      // 纬度对纹理的影响（极地更冷）
      const polarWeight = Math.pow(latFactor, 1.5)

      // 海拔噪声（多频率叠加）
      const elevation = fBm(nx * 20, ny * 20, 6, 3)
      // 大尺度大陆分布
      const continent = fBm(nx * 4 + 50, ny * 4, 4, 30)
      // 阈值：大陆 vs 海洋
      const continentThreshold = 0.38 + polarWeight * 0.15

      const isLand = continent > continentThreshold
      const localHeight = isLand ? elevation * (0.5 + (continent - continentThreshold) * 3) : 0
      ...
      if (isLand) {
        ...
        if (localHeight > 0.4) {
          // 高山（岩石/雪线以上）
          const snowAmount = Math.min(1, (localHeight - 0.4) * 4)
          r = 160 + (200 - 160) * snowAmount
          g = 150 + (200 - 150) * snowAmount
          b = 130 + (210 - 130) * snowAmount
        } else if (localHeight > 0.15) {
          // 山地/丘陵（棕灰色）
          r = 120 + localHeight * 100
          g = 100 + localHeight * 80
          b = 70 + localHeight * 60
        } else {
          // 低地/平原，纬度决定植被类型
          if (latFactor < 0.4) {
            ...
            g = 100 + elevation * 50
            b = 30 + elevation * 20
          } else if (latFactor < 0.7) {
            // 温带：混交林/草地
            const desertBlend = Math.max(0, (latFactor - 0.4) / 0.3)
            r = 60 + desertBlend * 80
            g = 90 + desertBlend * 40 - desertBlend * 30
            b = 30 - desertBlend * 10
          } else {
            // 极地：冻土/冰
            r = 180 + elevation * 30
        ...
        // 局部噪声微调
        const detail = fBm(nx * 60, ny * 60, 3, 4) * 10
        r += detail
        g += detail
        b += detail
      } else {
        // 海洋：深度决定颜色
        const depth = 0.5 - (continentThreshold - continent) * 2
        const clampedDepth = Math.max(0, Math.min(1, depth))
        if (clampedDepth < 0.3) {
          // 浅海（大陆架）
          r = 20 + clampedDepth * 30
          g = 80 + clampedDepth * 40
          b = 120 + clampedDepth * 30
        } else {
          // 深海
          r = 10 + clampedDepth * 5
          g = 40 + clampedDepth * 10
          b = 90 + clampedDepth * 50
        }
        // 浅海沙底纹理
        if (clampedDepth < 0.15) {
          const sand = fBm(nx * 30, ny * 30, 3, 5) * 15
          r += sand
          g += sand
          b += sand * 0.5
        }
