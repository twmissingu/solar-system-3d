...
大多数天文教育应用要么视觉效果惊艳但内容肤浅，要么像教科书一样枯燥无交互。**太阳系3D探索** 填补了这个空白。

核心理念：*"展示给你看 → 邀请你来思考。"* 每个模块的设计目标不是展示信息，而是激发问题——通过苏格拉底式追问链、模型透明度和动手实验。

你不需要记住火星的轨道周期是 687 天，而是**预测**它在 N 天后的位置去验证答案。你不需要被告知潮汐力的公式，而是**靠近**黑洞的事件视界去感受（数值上的）它的威力。
